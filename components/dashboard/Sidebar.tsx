...
import { signOut, type ApiUser } from '@/lib/api';
...
export default function Sidebar({ user }: SidebarProps) {
  ...
  return (
    <aside style={{ width: '220px', minHeight: '100vh', background: '#fff', borderRight: '1px solid #e8e6e1', display: 'flex', flexDirection: 'column', position: 'sticky', top: 0, height: '100vh' }}>
      <style>{`
        .sidebar-nav-item {
          display: flex; align-items: center; gap: 0.6rem;
          padding: 0.55rem 1rem; font-size: 0.875rem; font-weight: 400;
          color: #777; text-decoration: none; border-radius: 4px;
          margin: 0 0.5rem;
        ...
        .signout-btn {
          ...
          cursor: pointer; border-radius: 4px; margin: 0 0.5rem;
          width: calc(100% - 1rem); text-align: left;
          transition: color 0.12s, background 0.12s; letter-spacing: 0.01em;
        }
        .signout-btn:hover { color: #111; background: #f8f7f4; }
        .signout-btn:disabled { opacity: 0.4; cursor: not-allowed; }
      `}</style>

      <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '1.2rem 1.25rem', textDecoration: 'none', borderBottom: '1px solid #f0ede8', marginBottom: '0.5rem' }}>
      ...
      <nav style={{ flex: 1, paddingTop: '0.25rem' }}>
        {navItems.map(item => (
          <Link key={item.href} href={item.href} className={`sidebar-nav-item${pathname === item.href || pathname.startsWith(item.href + '/') ? ' active' : ''}`}>
            {item.icon}
            {item.label}
          </Link>
        ))}
      ...
      <div style={{ borderTop: '1px solid #f0ede8', padding: '0.75rem 0' }}>
        <div style={{ padding: '0.4rem 1.25rem 0.75rem', fontSize: '0.75rem', color: '#bbb', fontWeight: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {user?.email}
        </div>
