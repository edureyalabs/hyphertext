    ...
    <div style={{ minHeight: '100vh', background: '#f8f7f4', fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", display: 'flex', flexDirection: 'column' }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .header-nav-link {
          font-size: 0.8rem; font-weight: 400; color: #999;
          text-decoration: none; padding: 0.3rem 0.65rem;
          border-radius: 4px; transition: color 0.12s, background 0.12s; letter-spacing: 0.01em;
        }
        ...
        .account-btn {
          ...
          background: transparent; border: 1px solid #e8e6e1; border-radius: 6px;
          padding: 0.28rem 0.55rem 0.28rem 0.32rem; cursor: pointer;
        ...
        .avatar {
          ...
          font-size: 0.65rem; font-weight: 500; font-family: 'DM Mono', monospace; flex-shrink: 0;
        }
        ...
        .dropdown {
          ...
          animation: dropIn 0.15s ease both; z-index: 200;
        }
        @keyframes dropIn {
        ...
        .dropdown-item {
          ...
          width: 100%; padding: 0.65rem 1rem; font-size: 0.82rem;
          font-family: 'DM Sans', sans-serif; font-weight: 400; color: #444;
      ...
      <header style={{ height: '52px', background: '#fff', borderBottom: '1px solid #e8e6e1', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 1.5rem', flexShrink: 0, position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <nav style={{ display: 'flex', alignItems: 'center', gap: '0.2rem' }}>
          <Link href="/dashboard/projects" className={`header-nav-link${pathname?.startsWith('/dashboard/projects') ? ' active' : ''}`}>
            Projects
          </Link>
          <Link href="/explore" className={`header-nav-link${pathname?.startsWith('/explore') ? ' active' : ''}`}>
            Explore
          </Link>
        </nav>
        ...
        <div style={{ position: 'relative' }} ref={dropdownRef}>
          <button className={`account-btn${accountOpen ? ' open' : ''}`} onClick={() => setAccountOpen(v => !v)} aria-label="Account menu">
            ...
            <svg width="10" height="6" viewBox="0 0 10 6" fill="none" style={{ color: '#bbb', flexShrink: 0, transition: 'transform 0.15s', transform: accountOpen ? 'rotate(180deg)' : 'none' }}>
          ...
          {accountOpen && (
            <div className="dropdown">
              ...
              {username && (
                <Link href={`/u/${username}`} className="dropdown-item" onClick={() => setAccountOpen(false)}>
                  <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                    <circle cx="8" cy="8" r="6.5" stroke="currentColor" strokeWidth="1.2"/>
                    <circle cx="8" cy="6" r="2" stroke="currentColor" strokeWidth="1.2"/>
                    <path d="M3.5 13c0-2.485 2.015-4 4.5-4s4.5 1.515 4.5 4" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/>
                  </svg>
                  View profile
                </Link>
              )}
              ...
              <Link href="/account" className="dropdown-item" onClick={() => setAccountOpen(false)}>
                <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                  <path d="M8 1.5a2.5 2.5 0 100 5 2.5 2.5 0 000-5zM4 13c0-2.21 1.79-3.5 4-3.5s4 1.29 4 3.5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/>
                  <path d="M13 6l1.5 1.5L11 11l-2-.5.5-2L13 6z" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                Account settings
              </Link>
              ...
              <button className="dropdown-item signout" onClick={handleSignOut} disabled={signingOut}>
                <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
